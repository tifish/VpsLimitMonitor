...
- `open_stock_window` — open a provider inventory page in a new embedded browser window; optional `provider` selects NovixLink, HostYun, or CstoneCloud.
- `set_settings` — change `pollIntervalMinutes` / `alertRemainingPercent` / `novixLinkStockEnabled` / `hostYunStockEnabled` / `cstoneCloudStockEnabled`.
- `get_alerts` — recent toast alert records (verifies alerts fired without watching the screen).
- `check_stock` — force stock checks now and return plan availability; optional `provider` selects NovixLink, HostYun, or CstoneCloud.
